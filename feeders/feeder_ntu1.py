...
class Feeder(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # if self.split =='test':
        #     # 随机减少1-3个关节点
        #     num_joints_to_remove = random.randint(1, 3)
        #     indices_to_remove = random.sample(range(data_numpy.shape[2]), num_joints_to_remove)
        #     data_numpy[:, :, indices_to_remove, :] = 0  # 将选定关节点的所有值设置为0

        num_joints_to_remove = random.randint(1, 5)
        indices_to_remove = random.sample(range(data_numpy.shape[2]), num_joints_to_remove)
        data_numpy[:, :, indices_to_remove, :] = 0  # 将选定关节点的所有值设置为0
